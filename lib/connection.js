...
export async function createBaileysConnection(sessionId) {
  try {
    ...
    // Ensure auth directory exists before useMultiFileAuthState
    // Use config.AUTH_DIR which is writable in hosting environments
    const authDir = path.join(config.AUTH_DIR, sessionId);
    ...
    sock.ev.on("connection.update", async (update) => {
      ...
      // ========== CONNECTION OPENED ==========
        ...
        try {
          if (state?.creds) {
            const authPath = path.join(config.AUTH_DIR, String(sessionId));
...
`;
    ...
    sock.ev.on("creds.update", async () => {
      ...
      try {
        ...
        const authDir = path.join(config.AUTH_DIR, String(sessionId));
        try {
